...
DEBUG_MODE = "full"   # "fast" | "calibration" | "intermediate" | "medium" | "full" 
...
def evaluate_val(val_tensor, chunk_size=200_000):
    """
    Avalia val_loss em mini-batches para evitar OOM na GPU.
    chunk_size=200k é seguro para A40 48GB com rede até 512 neurônios.
    Se ainda der OOM, reduz para 100_000 ou 50_000.
    """
    n_total = tf.shape(val_tensor)[0].numpy()
    sum_sq = 0.0
    n_processed = 0
    ...
    for start in range(0, n_total, chunk_size):
        ...
        pred = model(chunk[:, :4], training=False)
        target = chunk[:, 4:8]
        sum_sq += float(tf.reduce_sum(tf.square(pred - target)))
        n_processed += (end - start)

    # n_features = 4 (u, v, w, P), igual ao tf.reduce_mean original
    n_features = 4
    return sum_sq / (n_processed * n_features)
